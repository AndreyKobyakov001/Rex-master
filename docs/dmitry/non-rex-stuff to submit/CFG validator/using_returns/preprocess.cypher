// SAME AS THE PREPROCESS SCRIPT IN MAIN, WITH AN ADDITION AT THE END TO ADD RETURN EDGES
...
// query against the lists `cfgBlocks` and `locations`
...
match (a)-[r:varWrite]->(b), (a)-[:edgeLink]->(edge:varWriteNode)-[:edgeLink]->(b), (f:cFunction) where f.id in r.locations
...
// Add return edges between CFG nodes:
//
// For each callsite, link the exit node of the callee to the *successors* of the callsite CFG

match (c:callNode)-[:edgeLink]->(target)-[:functionCFGLink]->()-[:contain*]->(end:cCFGBlock), p=(c)-[:cfgLink]->(:cCFGBlock)-[:contain]->(nextCFG:cCFGBlock)
    where end.id ends with ":CFG:0"
merge (end)-[:returns]->(nextCFG);
